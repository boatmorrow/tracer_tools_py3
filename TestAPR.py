...
print('Total 39Ar production rate is %1.3g,\
 ...
 from U,Th is %1.3g .'
          %(UC.APR.Ar39_value,UC.APR.P39Ar_ev_n,UC.APR.P39Ar_mu,UC.APR.P39Ar_alpha_n))
...
UC.calcAr_prod_rate_whole_rock()
UC.calc39ArSecEqul()
print('Equilibrium 39Ar concentration a surface is %1.3g' %UC.APR.Cseq[0])

#plot
fig,ax = plt.subplots()
ax.semilogx(UC.APR.P39Ar_ev_n,depth_r,'r--',label='$P^{39}Ar{_{n_{ev}}}$')
ax.semilogx(UC.APR.P39Ar_mu_n,depth_r,'b--',label='$P^{39}Ar{_{n_{\mu}}}$')
ax.semilogx(UC.APR.P39Ar_alpha_n,depth_r,'g--',label='$P^{39}Ar{_{n_{UTh}}}$')
ax.semilogx(UC.APR.P39Ar_mu,depth_r,'c--',label='$P^{39}Ar_{\mu}$')
...
#plot
fig,ax = plt.subplots()
ax.semilogx(UC.APR.Cseq,depth_r,'r--',label='n$_{ev}$')
...
ax.set_ylim(50,0)
#ax.set_xlim(10**-4,10**2)
ax.set_xlabel('$^{39}Ar_{eq}$ (atoms/g$_{rock}$')
ax.set_ylabel('Depth (m)')
ax.legend()
plt.show()
