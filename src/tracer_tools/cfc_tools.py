...
def download_hist_gas_conc(url='https://cdiac.ess-dive.lbl.gov/ftp/oceans/CFC_ATM_Hist/CFC_ATM_Hist_2015/CFC_atmospheric_histories_revised_2015_Table1.csv'):
    '''Grab historical  sf6 and cfc gas concentrations.  The URL will need to be updated periodically.  Data from:
        ...
        '''
    ddir = '~/.local/share/tracer_tools'
    fname = 'cfc_sf6_hist_atm.txt'
...
def download_gas_conc_hats(sf6_url='ftp://ftp.cmdl.noaa.gov/hats/sf6/combined/HATS_global_SF6.txt',cfc11_url='ftp://ftp.cmdl.noaa.gov/hats/cfcs/cfc11/combined/HATS_global_F11.txt',cfc12_url='ftp://ftp.cmdl.noaa.gov/hats/cfcs/cfc12/combined/HATS_global_F12.txt',cfc113_url='ftp://ftp.cmdl.noaa.gov/hats/cfcs/cfc113/combined/HATS_global_F113.txt'):
    ...
    ddir = '~/.local/share/tracer_tools'
    fname1 = 'sf6_atm.txt'
    fname2 = 'cfc11_atm.txt'
    fname3 = 'cfc12_atm.txt'
    fname4 = 'cfc113_atm.txt'

    if os.path.exists(ddir):
        fpath1=os.path.join(ddir,fname1)
        fpath2=os.path.join(ddir,fname2)
        fpath3=os.path.join(ddir,fname3)
        fpath4=os.path.join(ddir,fname4)
        subprocess.call(['curl','-o',fname1,sf6_url])
        subprocess.call(['curl','-o',fname2,cfc11_url])
        subprocess.call(['curl','-o',fname3,cfc12_url])
        subprocess.call(['curl','-o',fname4,cfc113_url])
    else:
        os.makedirs(ddir,exist_ok=True)
        fpath1=os.path.join(ddir,fname1)
        fpath2=os.path.join(ddir,fname2)
...
def get_cfc113_gas_conc_hats(fn='cfc113_atm.txt'):
    ddir = '~/.local/share/tracer_tools'
    if os.path.exists(ddir):
        f=os.path.join(ddir,fn)
...
def get_cfc11_gas_conc_hats(fn1='cfc11_atm.txt'):
    ddir = '~/.local/share/tracer_tools'
    if os.path.exists(ddir):
        f=os.path.join(ddir,fn)
        df = pd.read_csv(f ,header=0,comment='#',sep='\s+')
...
def get_cfc12_gas_conc_hats(fn='cfc12_atm.txt'):
    ddir = '~/.local/share/tracer_tools'
    if os.path.exists(ddir):
        f=os.path.join(ddir,fn)
        df = pd.read_csv(f,header=0,comment='#',sep='\s+')
...
def get_sf6_gas_conc_hats(fn='sf6_atm.txt'):
    ddir = '~/.local/share/tracer_tools'
    if os.path.exists(ddir):
        f=os.path.join(ddir,fn)
        df = pd.read_csv(f,header=0,comment='#',sep='\s+')
...
def compile_hats():
    ...
    fo = os.path.join(ddr,'cfc_sf6_update_atm.txt')
    df_atm.to_csv(fo)
    return df_atm
    

def get_hist_gas_conc(fn='cfc_sf6_hist_atm.txt'):
    ddir = '~/.local/share/tracer_tools'
    f = os.path.join(ddir,fn)
    if os.path.exists(f):
...
def create_atm_conc_csv():
    ...
    #write csv
    fo = os.path.join(ddir,'cfc_sf6_atm.txt')
    df_atm_full.to_csv(fo)
    return

def get_gas_conc(f='cfc_sf6_atm.txt'):
    data_path = os.path.join(os.path.dirname(__file__), f)
    try:
        df = pd.read_csv(data_path,index_col=0,parse_dates=True)
    except FileNotFoundError:
        print("updating gas concentrations.  only availabe for nix machines right now.")
        ddir = '~/.local/share/tracer_tools'
        create_atm_conc_csv()
        df = pd.read_csv(os.path.join(ddir,f),index_col=0,parse_dates=True)
    return df
